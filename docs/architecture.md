...
- 图片缓存按新鲜期、最长过期时间、`IMAGE_DISK_CACHE_ENTRIES` 条目数和磁盘容量清理；磁盘变更串行并在写入前预留容量，上游抓取受全局并发与等待队列硬限制。相关命中率、流量、容量和队列指标可在 `/api/health` 查看。
- 图片响应使用一年不可变浏览器缓存，并输出 `CDN-Cache-Control` / `Surrogate-Control`。配置 `IMAGE_CDN_BASE_URL` 后，请求先进入 CDN Pull Zone，CDN 回源时携带 `__origin=1` 防止重定向循环；前端在 CDN 加载失败时会自动使用源站路径重试。
- 酷安 EdgeOne 会拒绝第三方页面内的图片直链（HTTP 567），所以未配置自有 CDN 时继续使用白名单代理，而不是把浏览器直接重定向到酷安图片域名。
