...
- 修复话题详情从加载态切换到内容时，弹窗尺寸与位置明显跳动的问题。
- 详情弹窗统一预留稳定视口高度，并固定滚动条槽位，桌面端和移动端都不再闪烁。
- 话题排序改为保留现有内容原位刷新，失败时自动恢复原排序，不再用短骨架屏替换整段内容。
- 复用详情弹窗时自动回到顶部，并为动态、应用、话题、用户与收藏详情增加独立竞态保护，避免快速点击后显示旧内容。
