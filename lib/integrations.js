...
export const BATCH_MATCH_SCHEMA = {
  type: "object",
  additionalProperties: false,
  properties: {
    results: {
      ...
      items: {
        ...
        required: ["feedId", "matchScore", "reason", "evidence"],
      },
    },
  },
  required: ["results"],
...
export function normalizeBatchMatchResults(value = {}, feedIds = []) {
  const expected = new Set(feedIds.map(String));
  const rows = Array.isArray(value) ? value : value.results || value.evaluations || value.items;
  if (!Array.isArray(rows)) throw new Error("AI 批量结果缺少 results 数组");
  const results = new Map();
  for (const row of rows) {
    const feedId = String(row?.feedId ?? row?.feed_id ?? row?.id ?? "").trim();
    if (!expected.has(feedId) || results.has(feedId)) continue;
    results.set(feedId, normalizeMatchResult(row));
  }
  if (!results.size) throw new Error("AI 批量结果中没有可识别的帖子 ID");
  return results;
}
...
export function feishuNotificationTitle(feed = {}) {
  const originalTitle = compactPostText(feed.title);
  const username = compactPostText(feed.username);
  const normalizedTitle = originalTitle.replace(/\s+/g, "");
  const normalizedUsername = username.replace(/\s+/g, "");
  const syntheticTitle = !originalTitle
    || ["酷安动态", "用户动态", "动态"].includes(normalizedTitle)
    || (normalizedUsername && normalizedTitle === `${normalizedUsername}的动态`)
    || /^.{1,60}的动态$/u.test(normalizedTitle);
  if (!syntheticTitle) return originalTitle.slice(0, 200);

  const contentTitle = compactPostText(feed.message)
    .replace(/^(?:#[^#\r\n]{1,80}#\s*)+/u, "")
    .replace(/\s*查看链接\s*/gu, " ")
    .split(/\n+/)[0]
    .replace(/\s+/g, " ")
    .trim();
  return (contentTitle || "酷安内容").slice(0, 120);
}

export function buildFeishuFeedNotification(feed = {}, evaluation = {}) {
  const title = feishuNotificationTitle(feed);
